...
export default function index({ handleShow, showList }) {
	...
	return <div className={`${s.middle} bg-gray-300 pt_104 pb_200 z-10 w-screen flex items-center justify-center`}>
		<ul className={`${s.ul} h-100 w-50`}>
			...
			{
				Array.isArray(list) && list.length>0 && list.map((item,index) => <motion.li key={index} className={`${s.li}`} style={{zIndex:`${item.id==1?'100':'10'}`}} variants={variants} animate={item.id.toString()}  >{item.element}</motion.li>)
			}
		</ul>
	</div>
}

...
export default function index() {
  ...
  return (
    <div
      id="history"
      className={`w-100 p-0 ${s.wrap} flex justify-center items-center flex-col relative`}
    > 
      <div className={`${s.m_left} mobile:hidden top-0 bottom-0 absolute`}>
        <div className={`${s.m_line_wrap} absolute`}>
          {years.map((item,index)=><div onClick={() => handleClick(item.id)} key={item.id} style={{top:`${80*index}px`}} className={`${s.m_line_item} w-100 flex absolute items-center justify-between`} >
      ...
      <div className={`w-100 ${s.item_wrap} flex mobile:flex-col`}>
        <div
          className={`hidden mobile:flex ${s.item} items-center justify-center relative`}
        >
        ...
        <div
          className={`${s.history_wrap} w-100 flex flex-col mobile:flex-row items-end pr-20 mobile:pr-0 justify-center`}
        >
          <div className={`${s.history_item} flex flex-col`}>
            ...
            <div className={`${s.img_title} flex-1 flex flex-col bg-$white`}>
              ...
              <div className={`flex items-center`}>
                ...
                ></img>
                <span className="c_sub_title text-$86 mt-1">
                  {years.find((item) => item.active == true).id}年7月23日
                </span>
              </div>
            </div>
          </div>

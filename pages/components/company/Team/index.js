        ...
        {desc.length > 0 &&
          desc.map((item, index) => (
            <div key={index} className="flex  text-$86">
              <span className="span-point relative font_16 text-$86 pl-4 my-1">{item}</span>
            </div>
          ))}
      </div>
    </div>
...
const MangerCard = ({img_src='', name = "", title = "", desc = [] }) => {
  return (
    <div className={`${s.teamer} flex flex-row justify-around pl-4 items-center mb_43 mx-3`}>
      <img
        src={img_src}
      ...
      <div className="flex flex-col items-start pt-6 w_400 px-3 py-3 h-100 ">
        ...
        {desc.map((item, index) => (
            <span key={index} className="span-point relative font_16 text-$86 pl-4 my-1">{item}</span>
        ))}
      </div>
    </div>
  );
...
研发团队核心人员具有参与美国、德国相关的康复机器人研发经验，掌握核心关键技术。
      ...
      >
        <div
          className={`${s.ceo} flex flex-row justify-around pl-4 items-center mb_43`}
        >
          <img
            src="/assets/2560/company/lyw.png"
            className={`${s.img} relative`}
          ></img>
          <div className="flex flex-col items-start w_400 px-3 py-3 pt-6 h-100 ">
            <span className="font_30 font-black mb-1">李育文</span>
            <span className="font_20 text-$37 mb-1">CEO / 博士、东方学者</span>
              <span className="span-point relative font_16 text-$86 pl-3">清华大学本硕，加拿大麦吉尔大学MCGILL 博士</span>
              <span className="span-point relative font_16 text-$86 pl-3">加拿大普惠公司等8年项目研发和管理经验</span>
              <span className="span-point relative font_16 text-$86 pl-3">参与多项机器人与高端制造的跨国研发项目</span>
              <span className="span-point relative font_16 text-$86 pl-3">加拿大“工业研发基金奖”获得者</span>
          </div>
        </div>
        {managerList.map((item,index)=>(
         <MangerCard img_src={item.img_src} name={item.name} title={item.title} desc={item.desc} />
        ))}
      </div>

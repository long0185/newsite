...
export default function IregoNav() {
  ...
  return (
    <>
      <ul className="irego-nav  w-100 hidden  nav-pills mobile:flex justify-center bg-white" id="pills-tab" role="tablist">
        {list.map((item) => (
      ...
      <div className={`${s.m_wrap} mobile:hidden flex items-center justify-center`}>
        <div className="flex mt_54 items-center w_894 flex-wrap">
          {list.map((item) => (
            <div key={item.href} className={`nav-item ${s.nav_item} mx-2 mb-10 flex items-center justify-center ${href == item.href ? "bg-$primary text-white" : "text-$37 border-$af bg-$fa"}`} role="presentation">
